...
def _inc_torrent(amount):
    for torrent in response.ARGUMENTS["torrents"]:
        if not (
            "__moving" in torrent and torrent["__moving"]
        ):
            continue
...
def start_inc_worker(**kwargs):
    Thread(target=_inc_worker, kwargs=kwargs).start()
...
def create_app():
    ...
    start_inc_worker(
        amount=int(
            os.environ.get("MOCK_TRANS_AMOUNT", 2)
		),
        period=int(
            os.environ.get("MOCK_TRANS_PERIOD", 100)
        ),
    )
    return app
